...
impl ArchipelagoClient {
    ...
    pub async fn say(&mut self, message: &str) -> Result<(), ArchipelagoError> {
        self.send(ClientMessage::Say(Say { text: message.to_string() })).await
    }
     ...
     */
    pub async fn location_scouts(&mut self, locations: Vec<i64>, create_as_hint: i32) -> Result<LocationInfo, ArchipelagoError> {
